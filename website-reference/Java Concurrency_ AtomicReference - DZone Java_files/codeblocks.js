function CodeBlockVisitor() {
    ...
    _this.init = function() {
        if (_this.codeBlocks.length > 0) {
            ...
            loadScriptsSync([
                '/themes/dz20/lib/codemirror/lib/codemirror.js',
                '/themes/dz20/lib/codemirror/mode/meta.js'
            ]).then(function() {
                _this.process()
            })
        }
            ...
            if (mode) {
                ...
                if (mode.mode == 'htmlmixed') {
                    ...
                    for (var j = 0; j < types.length; j++) {
                        load.push('/themes/dz20/lib/codemirror/mode/' + types[j] + '/' + types[j] + '.js')
                    }
                } else if (mode.mode != 'null') {
                    load.push('/themes/dz20/lib/codemirror/mode/' + mode.mode + '/' + mode.mode + '.js')
                }
